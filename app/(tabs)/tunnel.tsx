...
export default function TunnelScreen() {
  // CHECK [RETO 03 - PASO 1]: Declara el estado booleano 'tunelActivo' inicializado en false usando useState(false)
  const [tunelActivo, setTunelActivo] = useState(false);

  // CHECK [RETO 03 - PASO 2]: Programa la función toggleTunel para alternar entre true y false
  const toggleTunel = () => {
    // CHECK: Alternar el estado tunelActivo llamando a setTunelActivo(!tunelActivo)
      ...
      <ScrollView className="flex-1" contentContainerClassName="px-5 pt-4 pb-36">
        ...
        >
          <Text className="text-xs font-bold text-black leading-relaxed mb-3">
            <Text className="font-bold">Misión:</Text> Abre este archivo (
            <Text className="font-mono text-blue-700 font-bold">
              app/(tabs)/tunnel.tsx
            </Text>
            ). Descomenta y utiliza el hook{' '}
            <Text className="font-mono text-pink-700 font-bold">
              useState(false)
            </Text>{' '}
            para crear el estado reactivo{' '}
            <Text className="font-mono font-bold">
              [tunelActivo, setTunelActivo]
            </Text>.
          </Text>
          ...
          <View className="bg-white/80 p-3 rounded-lg border-2 border-black gap-1.5">
            ...
            <Text className="text-xs font-medium text-black">
              1. Conecta la función{' '}
              <Text className="font-mono font-bold">toggleTunel</Text> al botón
              de abajo.
            </Text>
            <Text className="text-xs font-medium text-black">
              2. Al pulsar el botón, la tarjeta debe cambiar a verde
              ("TÚNEL CONECTADO").
            </Text>
            <Text className="text-xs font-medium text-black">
        ...
        >
          <View className="flex-row items-center gap-2 mb-3">
            ...
            <Text className="text-sm font-black text-black uppercase">
              {tunelActivo
                ? 'Conectado vía Expo Cloud'
                : 'Bloqueado por Client Isolation'}
            </Text>
          </View>
          ...
          <View className="bg-white/80 p-3 rounded-lg border-2 border-black mb-3 flex-row items-center gap-2">
            ...
            <Text className="font-extrabold text-xs text-black">
              URL:{' '}
              {tunelActivo
                ? 'exp://bar-salesiano.exp.direct:80'
                : 'exp://192.168.10.45:8081 (Sin Conexión)'}
            </Text>
          </View>
